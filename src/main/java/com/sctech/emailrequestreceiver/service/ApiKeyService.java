...
public class ApiKeyService {
    ...
    public Optional<Authentication> validateApiKey(HttpServletRequest request) throws IOException {
        ...
        if(requestApiKey.length() < KEY_LENGTH || requestApiKey.length() > MAX_API_KEY_LENGTH || !requestApiKey.matches(API_KEY_REGEX)){
        ...
        if(company.getStatus().equals(EntityStatus.INACTIVE) || company.getCredits() <= 0){
            return Optional.empty();
        }
        ...
        MDC.put(AppHeaders.ENTITY_ID,company.getId());
        MDC.put(AppHeaders.ENTITY_TYPE,company.getBillType().name());
        MDC.put(AppHeaders.ENTITY_CHANNEL_NAME,apiKeyEntity.getName());
